...
// Hot-RAM 2-bit benchmark with explicit hash materialization.
// XSeal: scan (pos only) + xseal_harvest_kmers<true> (latest harvest layer).
// Protocol matches xseal_hot_bench / simd_min_hot_hash_bench.
...
uint64_t digest_pos_hash(const uint32_t *pos, const uint64_t *hash, size_t hits) {
  uint64_t d = 0;
...
size_t buf_capacity_for_thread(size_t num_bases, int num_threads) {
...
RunResult run_closed_sync(const uint8_t *data, size_t num_bases, int sync_k, int s,
                           int harvest_k, int num_threads, uint32_t **out_pos,
                           uint64_t **out_hash, int internal_loops, int warmup_loops) {
    ...
    threads.emplace_back([&, t]() {
      ...
      for (int iter = 0; iter < internal_loops; ++iter) {
        ...
        if (thread_num_bases >= 256) {
          ...
          const uint8_t *enc_chunk = data + (start_base / 4);
          thread_hits[t] = scanner.scan<W, 0, true, true>(
              chunk_ptr, num_chunks, out_pos[t], thread_num_bases);
          if (thread_hits[t] > 0) {
            xseal_harvest_kmers<true>(enc_chunk, out_pos[t], thread_hits[t], harvest_k,
                                      out_hash[t]);
          }
        } else {
          thread_hits[t] = 0;
        }
        asm volatile("" : : "g"(thread_hits[t]), "g"(out_pos[t]), "g"(out_hash[t]) : "memory");
  ...
  for (int iter = 0; iter < internal_loops; ++iter) {
    const size_t cap = buf_capacity_for_thread(num_bases, num_threads);
    for (int t = 0; t < num_threads; ++t) {
      std::memset(out_pos[t], 0, cap * sizeof(uint32_t));
      std::memset(out_hash[t], 0, cap * sizeof(uint64_t));
    }
    ...
    for (int t = 0; t < num_threads; ++t) {
      ...
      digest += digest_pos_hash(out_pos[t], out_hash[t], thread_hits[t]);
    }
...
RunResult dispatch_scheme(Scheme scheme, int w_sync, const uint8_t *data, size_t num_bases,
                         int sync_k, int s, int harvest_k, int min_k, int min_w,
                         int num_threads, uint32_t **out_pos, uint64_t **out_hash,
                         int internal_loops, int warmup_loops) {
  switch (scheme) {
    case Scheme::ClosedSync:
      return run_closed_sync<21>(data, num_bases, sync_k, s, harvest_k, num_threads,
                                 out_pos, out_hash, internal_loops, warmup_loops);
    case Scheme::OpenSync:
      return run_open_sync<21, 10>(data, num_bases, sync_k, s, harvest_k, num_threads,
                                   out_pos, out_hash, internal_loops, warmup_loops);
    case Scheme::Minimizer:
      return run_minimizer<21>(data, num_bases, min_k, min_w, harvest_k, num_threads,
                               out_pos, out_hash, internal_loops, warmup_loops);
  }
  return {};
}
...
void print_scheme(Scheme scheme, int num_threads, size_t num_bases, double mean,
                  double inner_std, const RunStats &stats) {
  const double coverage = static_cast<double>(stats.hits) / static_cast<double>(num_bases);
  std::cout << std::fixed << std::setprecision(6);
  std::cout << "INNER|XsealHotHash|" << scheme_name(scheme) << "|" << num_threads << "|" << mean
            << "|" << inner_std << "|" << stats.hits << "|" << coverage << "|"
            << std::hex << std::setw(16) << std::setfill('0') << stats.digest << std::dec
            << "\n";
  std::cout << std::setprecision(4);
  std::cout << "RESULT|XsealHotHash|" << scheme_name(scheme) << "|" << num_threads << "|"
...
int main(int argc, char **argv) {
  if (argc < 3) {
    std::cerr << "Usage: " << (argc > 0 ? argv[0] : "xseal_hot_hash_bench")
              << " <hg38_2bit.bin> <threads> [sync_k=31] [s=11] [min_w=21]\n";
    return 1;
  }
  ...
  uint64_t **out_hash = new uint64_t *[num_threads];
  for (int t = 0; t < num_threads; ++t) {
    out_pos[t] = static_cast<uint32_t *>(aligned_alloc(64, (cap + 256) * sizeof(uint32_t)));
    out_hash[t] = static_cast<uint64_t *>(aligned_alloc(64, (cap + 256) * sizeof(uint64_t)));
    if (!out_pos[t] || !out_hash[t]) {
  ...
  std::cout << std::fixed;
  std::cout << "NOTE|XsealHotHash|sync_k=" << sync_k << "|s=" << s << "|w_sync=" << w_sync
  ...
  for (Scheme scheme : schemes) {
    RunResult result = dispatch_scheme(scheme, w_sync, data, num_bases, sync_k, s, harvest_k,
                                       min_k, min_w, num_threads, out_pos, out_hash,
                                       kInternalLoops, kWarmupLoops);
