//! Hot-RAM 2-bit packed benchmark with pos + 64-bit hash output.
//! simd-minimizers: scan positions, then materialize hashes via Output::values_u64().
...
fn buf_capacity(num_bases: usize, num_threads: usize) -> usize {
    std::cmp::max(1024 * 1024 * 64, num_bases / num_threads / 4 + 1024)
}

fn digest_pos_hash(pos: &[u32], hash: &[u64]) -> u64 {
    let mut d = 0u64;
...
fn run_scheme(
    ...
    min_k: usize,
    min_w: usize,
    thread_pos: &mut [Vec<u32>],
    thread_hash: &mut [Vec<u64>],
) -> RunStats {
    let num_threads = thread_pos.len();
    let chunk_size_bases = (num_bases / num_threads / 128) * 128;

    let start = Instant::now();

    let partial: Vec<ThreadStats> = thread_pos
        .par_iter_mut()
        .zip(thread_hash.par_iter_mut())
        .enumerate()
        .map(|(t, (pos, hashes))| {
            pos.clear();
            hashes.clear();
            ...
            if thread_num_bases < 512 {
                return ThreadStats { hits: 0, digest: 0 };
            }
            ...
            match scheme {
                Scheme::ClosedSync => {
                    let out = canonical_closed_syncmers(lib_s, w).run(packed_seq, pos);
                    hashes.extend(out.values_u64());
                }
                Scheme::OpenSync => {
                    let out = canonical_open_syncmers(lib_s, w).run(packed_seq, pos);
                    hashes.extend(out.values_u64());
                }
                Scheme::Minimizer => {
                    let out = canonical_minimizers(min_k, min_w).run(packed_seq, pos);
                    hashes.extend(out.values_u64());
                }
            }

            debug_assert_eq!(pos.len(), hashes.len());
            let hits = pos.len();
            black_box(hits);
            black_box(pos.as_ptr());
            black_box(hashes.as_ptr());
    ...
    let per_thread: Vec<ThreadStats> = thread_pos
        .par_iter()
        .zip(thread_hash.par_iter())
        .zip(partial.par_iter())
        .map(|((pos, hashes), p)| {
            let digest = digest_pos_hash(pos, hashes);
            black_box(digest);
            ThreadStats {
                hits: p.hits,
                digest,
...
fn bench_scheme(
    ...
    for iter in 0..INTERNAL_LOOPS {
        let stats = run_scheme(
            scheme, data, num_bases, lib_s, w, min_k, min_w, thread_pos, thread_hash,
        );
...
fn main() {
    ...
    if args.len() < 3 {
        eprintln!(
            "Usage: {} <hg38_2bit.bin> <threads> [sync_k=31] [s=11] [min_w=21]",
            args.first().map(String::as_str).unwrap_or("simd_min_hot_hash_bench")
        );
        std::process::exit(1);
    }
    ...
    let cap = buf_capacity(num_bases, num_threads);
    let mut thread_pos: Vec<Vec<u32>> = (0..num_threads).map(|_| Vec::with_capacity(cap)).collect();
    let mut thread_hash: Vec<Vec<u64>> =
        (0..num_threads).map(|_| Vec::with_capacity(cap)).collect();

    println!(
        "NOTE|SimdMinHotHash|sync_k={}|s={}|w={}|min_k={}|min_w={}|bases={}|loops={}|warmup={}|recorded={}",
    ...
    for scheme in [
        ...
        println!(
            "INNER|SimdMinHotHash|{}|{}|{:.6}|{:.6}|{}|{:.8}|{:016x}",
            scheme.name(),
            num_threads,
            mean,
            std,
