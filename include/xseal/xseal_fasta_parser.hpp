...
/**
 * @brief Core FASTA scanning engine
 * Responsible for identifying sequence segments and generating the Run-Length Table.
 * Template-free, extreme performance, easily extensible for QC and Profiling features.
 */
...
public:
    ...
    void parse(const uint8_t *data, size_t size, uint64_t absolute_offset,
        ...
        // Process potentially truncated physical segments at the end of the chunk
...
private:
    ...
    inline void handle_header(const uint8_t *data, size_t size, size_t &ptr,
        ...
        // If physical bytes is 0 or 1 (only '>'), it means this is the start of a new Header
        ...
        while (ptr < size) {
            ...
            if (in_header_id) {
                size_t i = 0;
                // If it's the very beginning of the segment and the first character is '>', skip it and don't count it in ID length
                ...
                // If end of line is reached without a space, the ID ends here (or the ID scan for this segment is paused)
                if (nl && in_header_id) {
                    if (current_id_scan_len > max_id_len) max_id_len = current_id_scan_len;
                    in_header_id = false;
                }
...
/**
 * @brief Compatibility layer: convert table-based scanning to the original callback pattern
 */
...
public:
    XsealFastaParserT(BasesHandler b, HeaderHandler h, GapHandler g)
        ...
        run_table_cache.reserve(1024); // Pre-allocate some space
    }
    ...
    void parse(const uint8_t *data, size_t size, uint64_t absolute_offset,
        ...
        // Compatibility layer: simulate callbacks
